...
import {
  fetchCryptoPrices, formatCryptoAmount, formatCryptoValue,
  TOP_CRYPTOS, type CryptoPrice, type CryptoId,
} from "@/lib/utils/coingecko";
import {
  Plus, RefreshCw, Trash2, Pencil, TrendingUp, TrendingDown,
  Wallet2, Bitcoin, ArrowUpDown,
} from "lucide-react";
...
export function CryptoPage() {
  ...
  const emptyForm = { coin_id: "bitcoin" as string, quantity: "", avg_buy_price: "", notes: "" };
  ...
  function openEdit(h: CryptoHolding) {
    setEditingId(h.id);
    setForm({ coin_id: h.coin_id, quantity: String(h.quantity), avg_buy_price: String(h.avg_buy_price), notes: h.notes || "" });
    setFormError("");
  }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (isNaN(quantity) || quantity <= 0) { setFormError(t("invalidQuantity")); setSaving(false); return; }
    if (isNaN(avgBuyPrice) || avgBuyPrice < 0) { setFormError(t("invalidAvgPrice")); setSaving(false); return; }

    const coin = TOP_CRYPTOS.find((c) => c.id === form.coin_id);
    if (!coin) { setFormError(t("invalidCrypto")); setSaving(false); return; }

    const payload = {
      coin_id: coin.id, symbol: coin.symbol, name: coin.name,
      quantity, avg_buy_price: avgBuyPrice, notes: form.notes || null,
    };
    ...
    if (editingId) {
      ...
      const { error } = await supabase.from("crypto_holdings").insert({ ...payload, workspace_id: wsId, user_id: user!.id });
      if (error) { setFormError(error.message); setSaving(false); return; }
    }
    ...
    setForm(emptyForm);
    setSaving(false);
    toast(editingId ? t("cryptoUpdated") : t("cryptoAdded"));
  ...
  return (
    <div className="animate-fade space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="bg-card border border-border rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">{t("currentValue")}</span>
          </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-1">
            {portfolio.totalPnl >= 0 ? <TrendingUp className="h-4 w-4 text-emerald-500" /> : <TrendingDown className="h-4 w-4 text-destructive" />}
            <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">{t("profitLoss")}</span>
          </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">{t("profitability")}</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-card border border-border rounded-2xl p-6">
          <h3 className="font-semibold text-foreground mb-5 flex items-center gap-2">
            ...
            {editingId ? t("editPosition") : t("addCrypto")}
          </h3>
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">{t("cryptoAsset")}</label>
              <select value={form.coin_id} onChange={(e) => setForm({ ...form, coin_id: e.target.value })}
                ...
                {TOP_CRYPTOS.map((c) => {
                  ...
                  return (<option key={c.id} value={c.id}>{c.symbol} — {c.name} {price ? `(${formatCryptoValue(price.current_price)})` : ""}</option>);
                })}
              </select>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">{t("quantity")}</label>
                <input type="text" required value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })}
                  placeholder="Ex: 0.5"
              ...
              <div>
                <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">{t("avgPrice")}</label>
                <input type="text" required value={form.avg_buy_price} onChange={(e) => setForm({ ...form, avg_buy_price: e.target.value })}
                  placeholder="0,00"
                  className="w-full bg-background border border-border rounded-lg px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20" />
              </div>
            </div>
                ...
                return (
                  <div className="p-3 rounded-xl bg-muted/50 border border-border text-xs space-y-1">
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{t("positionCurrentValue")}</span>
                      <span className="font-bold text-foreground">{formatCryptoValue(value)}</span>
                    </div>
                    {avgPrice > 0 && (
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">{t("positionPnl")}</span>
            ...
            <div>
              <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">{t("notes")}</label>
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button type="submit" disabled={saving || !permissions.canEdit}
                ...
                {saving ? t("saving") : editingId ? t("saveChanges") : t("addPosition")}
              </button>
            </div>
          </form>
        </div>
        ...
        <div className="bg-card border border-border rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-border flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">{holdings.length} {t("asset")}{holdings.length !== 1 ? "s" : ""}</span>
          </div>

          <div className="px-6 py-2 border-b border-border grid grid-cols-5 gap-2">
            <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">{t("asset")}</span>
            <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider text-right">{t("qty")}</span>
            <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider text-right">{t("currentPrice")}</span>
            <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider text-right">{t("valuePnl")}</span>
          ...
          {portfolio.items.length === 0 ? (
            <div className="px-6 py-16 text-center">
              ...
              <p className="text-sm font-semibold text-foreground mb-1">{t("noHoldings")}</p>
              <p className="text-xs text-muted-foreground">{t("noHoldingsDesc")}</p>
            </div>
          ) : (
            <div className="divide-y divide-border">
              {portfolio.items.map((item) => (
                <div key={item.id} className="px-6 py-3 grid grid-cols-5 gap-2 items-center group hover:bg-muted/30 transition-colors">
                  ...
                  <p className="text-xs text-foreground text-right font-mono">{formatCryptoAmount(item.quantity, item.symbol)}</p>
                  <div className="text-right">
                  ...
                  <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => openEdit(item)} className="h-6 w-6 flex items-center justify-center rounded hover:bg-secondary text-muted-foreground hover:text-foreground transition-colors" aria-label={tc("edit")}>
                      <Pencil className="h-3 w-3" />
                    </button>
                    <button onClick={() => { setDeletingId(item.id); setDeleteModalOpen(true); }} className="h-6 w-6 flex items-center justify-center rounded hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors" aria-label={tc("delete")}>
      ...
      {Object.keys(prices).length > 0 && (
        <div className="bg-card border border-border rounded-2xl p-5">
          <h3 className="font-semibold text-foreground text-sm mb-4">{t("livePrices")}</h3>
